// 제이슨 데이터로 변환한뒤 해당 url로 데이터를 보낸다음 반환 데이터와 콜백펑션을 리턴한다
function sendRequest(url, data, callback, isAserver, type, dataType) {
	if( isAserver == null ) {
		isAserver = true;
	}
	if( type == null ) {
	...
	$.ajaxSetup({
		...
		error:function(xhr, textStatus){
			...
			if( textStatus == 'timeout' ) {
				alert('인터넷 연결시간이 초과되었습니다. 잠시 후 다시 시도해주세요.');
			} else if( textStatus == 'error' ) {
				alert('인터넷 연결이 불안합니다. 인터넷 연결을 확인해주세요.');
			} else if( textStatus == 'abort' ) {
				alert('인터넷 연결이 거절되었습니다. 문제가 계속될 경우 관리자에게 문의해주세요.');
			} else if( textStatus == 'parsererror' ) {
				alert('수신된 데이터가 올바르지 않습니다. 문제가 계속될 경우 관리자에게 문의해주세요.');
			}
...
function showLd() {
	...
	bval=getBusyOverlay('viewport',{color:'gray',opacity:0.25});

	setTimeout(function(){hideLd();}, 10000);
}
...
function setPagination(totalCnt, offset) {
	var pageCnt = Math.ceil(totalCnt / offset);

	var pageDiv = $('<div />').attr('class', 'pagination');
	$('body').append(pageDiv);

	var first = $('<a />').attr('style', "text-decoration:none").attr('href', 'javascript:onPage(1);').html('<< First ');
	pageDiv.append(first);

	for(var i=1; i<=pageCnt; i++) {
		var pageLink = $('<a />').attr('style', "text-decoration:none").attr('href', 'javascript:onPage(' + i +');').html(i + ' ');
		pageDiv.append(pageLink);
	}

	var last = $('<a />').attr('style', "text-decoration:none").attr('href', 'javascript:onPage(' + pageCnt + ');').html('Last >>');
	pageDiv.append(last);
}
